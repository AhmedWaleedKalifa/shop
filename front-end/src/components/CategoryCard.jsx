...
function CategoryCard({ id, leftImage, rightImage, name, buttonText }) {
  return (
   <Link 
    to={`/category/${id}`} 
    className="flex flex-col md:flex-row w-full md:aspect-auto  relative font-body"
    aria-label={`Browse ${name} category`}
>
    {/* Left Image Section */}
    <figure 
        className="w-full md:w-1/2 relative bg-cover bg-center md:bg-no-repeat  aspect-2/3 "
        style={{ backgroundImage: `url(${leftImage})` }}
    />
    
    {/* Right Image Section - Hidden on mobile, shown on desktop */}
    <figure 
        className="hidden md:block md:w-1/2 bg-cover bg-no-repeat  aspect-2/3"
