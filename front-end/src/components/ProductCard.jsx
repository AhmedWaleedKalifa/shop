...
function ProductCard({ name, id, description, price, discount, image }) {
  return (
   <Link 
    to={`/product/${id}`} 
    className='relative left-0 w-[calc(50%-4px)] md:w-[calc(25%-6px)] lg:w-[calc(12.5%-8px)] h-auto bg-gray/4 flex flex-col justify-between font-body'
    aria-label={`View ${name} product details`}
>
    <article className="aspect-3/4 overflow-hidden relative">
        {discount !== 0 && (
            <div className='rounded-full p-2 absolute top-2 left-2 w-11 h-11 bg-yellow flex flex-row items-center justify-center text-lg font-bold border text-gray font-heading'>
    ...
    <h3 className="p-2 text-base font-semibold line-clamp-1 font-body-medium">
        {name}
    </h3>
    
    <p className="px-2 line-clamp-1 text-ellipsis overflow-x-hidden text-sm text-gray font-body">
        {description}
    </p>
